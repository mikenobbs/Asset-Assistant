...
class MediaMatcher:
    ...
    def _find_movie_match(self, movie_name, movie_year):
        ...
        for dir_name in os.listdir(self.movies_dir):
    ...
    def _find_show_directory(self, show_name, show_year):
        ...
        for dir_name in os.listdir(self.shows_dir):
            ...
            if dir_match:
                ...
                if show_name.lower() == dir_show_name.lower() and show_year == dir_show_year:
                    return dir_name
        ...
        for dir_name in os.listdir(self.shows_dir):
            dir_match = re.match(r'(.+)\s\((\d{4})\)', dir_name, re.IGNORECASE)
            if dir_match:
                dir_show_name = dir_match.group(1).strip().lower()
                
                for show_var in show_variants:
                    if show_var in dir_show_name or dir_show_name in show_var:
